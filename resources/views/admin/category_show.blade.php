...
@section('content')
    ...
    {{--  <section class="content-header">
					<div class="container-fluid my-2">
						<div class="row mb-2">
							<div class="col-sm-6">
								<h4>Livetv Category</h4>
							</div>
							<div class="col-sm-6 text-right">
								<a href="{{ route('category.category_add') }}" class="btn btn-success">New Category</a>
							</div>
						</div>
					</div>
					<!-- /.container-fluid -->
        ...
        <div class="container p-auto">
            <div class="card">
                <div class="card-header">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h5>Livetv Category</h5>
                        </div>
                        <div class="col-sm-6 text-right a_btnn">
                            <a href="{{ route('category.category_add') }}"
                                style="font-size: 0.812rem;
                                        border-radius: 15px;"
                                class="pull-right btn btn-success btn-lg"><i class="fa fa-plus"></i> Add New</a>
                        </div>
                    </div>
                    <hr>
                        ...
                        <tbody>
                            @foreach ($data as $d)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('category.category_update', $d->id) }}">
                                        ...
                                        <a href="{{ route('category.category_delete', $d->id) }}"
    ...
    <script>
        function toggleStatus(rowId) {
            $.ajax({
                url: '{{ route('category.update-status') }}' + '?id=' + rowId,
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: '{{ csrf_token() }}'
